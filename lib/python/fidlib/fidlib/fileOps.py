...
class TemporaryFilePath(object):
    ...
    """
    def __init__(self, prefix=None, suffix="tmp", tmp_dir=None, is_martian=False):
        self.path = get_tmp_file(prefix=prefix, suffix=suffix, tmp_dir=tmp_dir, is_martian=is_martian)
...
def get_tmp_file(prefix=None, suffix="tmp", tmp_dir=None, is_martian=False):
    """
    ...
    :param is_martian: Are we running this in the Martian environment? If so, get the right path for this job
    :return: A file path.
    """
    if tmp_dir is None:
    ...
    if is_martian:
        base_path = martian.make_path(base_path)
    while True:
...
def print_iterable(fspec, item_iter, add_newline=False):
    """
    Convenience function that simply writes an iterable of lines to fspec (file handle or file)
    :param fspec: A open file handle or file path
    :param item_iter: One or more things to write.
    """
    fh = _resolve_fspec(fspec, 'w')
    for line in item_iter:
